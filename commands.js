// ─── commands.js ─────────────────────────────────────────────────────────────
// Registra los slash commands. Ejecuta con: node commands.js
...
const commands = [
    ...
    {
        name: 'pedidos',
        description: 'Historial de pedidos registrados.',
        options: [
            {
    ...
    },
    {
        ...
        options: [
            { name: 'id', type: ApplicationCommandOptionType.Integer, description: 'Número de orden a cancelar', required: true }
        ]
    },
    {
        name: 'exportar',
    ...
    // ── Tickets ───────────────────────────────────────────────────────────
    {
        ...
        options: [
            ...
            {
                name: 'pedidos', type: ApplicationCommandOptionType.Channel,
                description: 'Canal donde se registran las órdenes de Compra', required: false,
                channel_types: [ChannelType.GuildText]
            },
            {
    ...
    // ── Reputación ────────────────────────────────────────────────────────
    {
        ...
        options: [
            { name: 'id', type: ApplicationCommandOptionType.Integer, description: 'Número de orden a valorar', required: true }
        ]
    },
    {
        name: 'resenas',
    ...
    {
        name: 'configdm',
        description: 'Activa o desactiva los DMs automáticos (solo admins).',
        options: [
...
async function deleteCommandsInRoute(route) {
    const existing = await rest.get(route).catch(() => []);
    const list = Array.isArray(existing) ? existing : [];
    await Promise.all(list.map(cmd => rest.delete(`${route}/${cmd.id}`).catch(() => {})));
}

async function syncCommands(targetRoute, targetLabel) {
    const existing = await rest.get(targetRoute).catch(() => []);
    const currentNames = new Set(commands.map(cmd => cmd.name));

    const stale = (Array.isArray(existing) ? existing : []).filter(cmd => !currentNames.has(cmd.name));
    await Promise.all(stale.map(cmd => rest.delete(`${targetRoute}/${cmd.id}`).catch(() => {})));

    await rest.put(targetRoute, { body: commands });
    console.log(`✅ ${commands.length} comandos registrados correctamente ${targetLabel}.`);
}
...
(async () => {
    try {
        ...
        if (guildId) {
            // Limpiar ambos ámbitos para evitar duplicados legacy/global + guild
            await deleteCommandsInRoute(Routes.applicationCommands(clientId));
            await deleteCommandsInRoute(Routes.applicationGuildCommands(clientId, guildId));
            await syncCommands(Routes.applicationGuildCommands(clientId, guildId), `en el servidor ${guildId}`);
        } else {
            await deleteCommandsInRoute(Routes.applicationCommands(clientId));
            await syncCommands(Routes.applicationCommands(clientId), 'a nivel global');
        }
    } catch (error) {

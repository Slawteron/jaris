// ─── ui/panels.js ──────────────────────────────────────────────────────────
// PREMIUM VERSION - Matched to Aurex Bot Quality Standards
const {
...
const PREMIUM_COLOR = '#DC143C'; // Crimson (más vibrante)
const ACCENT_GOLD = '#FFD700'; // Dorado para premium

// ─── Categorías del panel principal (con descripciones mejoradas) ─────────
const CATEGORIAS = {
    compra: {
        emoji: '🛒',
        label: 'Comprar',
    ...
    otro: {
        emoji: '🔔',
        label: 'Otro',
        desc: '¿Otra consulta no encaja en las opciones anteriores?',
        icon: '💬'
    },
};
    ...
    const menu = new StringSelectMenuBuilder()
        ...
        .addOptions(Object.entries(CATEGORIAS).map(([value, c]) =>
            new StringSelectMenuOptionBuilder()
                .setLabel(`${c.label}`)
                .setDescription(`${c.desc.slice(0, 90)}...`)
                .setEmoji(c.emoji)
                .setValue(value)
        ));
...
function buildMetodoSelect(customId = 'ir_metodo', pais = null) {
    ...
    ].map(o => ({ ...o, emoji: o.emoji ?? '❔' })).filter(o => !o.soloArgentina || pais === 'argentina');
...
// ─── Botones: confirmar / volver / cancelar (PREMIUM VERSION) ──────────────
